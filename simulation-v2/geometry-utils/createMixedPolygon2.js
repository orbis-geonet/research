...
export function createMixedPolygon2(place1, place2) {
  ...
  if (isOverlapping) {
    ...
    const angle2 = Math.atan2(intersectionPoints[1].y - place1.y, intersectionPoints[1].x - place1.x);

    // Angle from center of place2 to intersection point
    const angle4 = Math.atan2(intersectionPoints[1].y - place2.y, intersectionPoints[1].x - place2.x);

    const arcPoints1 = calculateArcPoints({ x: place1.x, y: place1.y }, place1.radius, angle2, Math.atan2(p1b.y - place1.y, p1b.x - place1.x), 20);
    const arcPoints2 = calculateArcPoints({ x: place2.x, y: place2.y }, place2.radius, Math.atan2(p2b.y - place2.y, p2b.x - place2.x), angle4, 20);
